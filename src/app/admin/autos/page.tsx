...
import { formatGhsPrice, CONDITION_CONFIG } from '@/lib/autoUtils'
      ...
      <div className="rounded-2xl border border-gray-100 bg-white shadow-sm">
        <div className="border-b border-gray-100 px-5 py-4 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="relative">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search title, make or seller..."
        ...
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-gray-500 text-sm">No auto listings yet</p>
          </div>
        ) : (
                  ...
                  <tr key={l.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    <td className="px-4 py-3">
                      <a href={`/autos/${l.id}`} target="_blank" rel="noopener noreferrer"
                        className="font-medium text-gray-900 hover:text-[#E63946] hover:underline line-clamp-1 max-w-[200px] block">
                        {l.year} {l.make} {l.model}
                      </a>
                      <p className="text-xs text-gray-400 mt-0.5 capitalize">{l.listing_type} · {l.vehicle_type}</p>
                    </td>
                    <td className="px-4 py-3 text-gray-600">{l.profiles?.username ?? '—'}</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${CONDITION_CONFIG[l.condition]?.color ?? 'bg-gray-100 text-gray-600'}`}>
                        {CONDITION_CONFIG[l.condition]?.label ?? l.condition}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <input
                        ...
                        className="w-20 border border-gray-200 rounded px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-[#E63946]"
                      />
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => toggleFeatured(l.id, l.featured)} disabled={updating === l.id}
                        className={`rounded-full p-1.5 transition-colors ${l.featured ? 'bg-[#E63946] text-white' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}>
