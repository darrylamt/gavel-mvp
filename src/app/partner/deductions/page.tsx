...
import { Sliders, AlertCircle, CheckCircle2, Pencil, Check } from 'lucide-react'
...
function NumInput({ value, onChange, placeholder }: NumberInputProps) {
  return (
    <input
      ...
      className="w-20 rounded-lg border border-blue-200 bg-white px-2 py-1 text-xs focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-100"
    />
  )
}
...
export default function PartnerDeductionsPage() {
              ...
              <tbody className="divide-y divide-gray-50">
                {rates.map((row) => {
                  ...
                  return (
                    <tr
                      key={row.id}
                      className={`transition-colors ${isEditing ? 'bg-blue-50/40' : 'hover:bg-blue-50/20'}`}
                    >
                      ...
                      <td className="px-5 py-3.5 text-right">
                        {isEditing ? (
                          ...
                          <button
                            ...
                            disabled={actionBusy}
                            className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 hover:border-blue-300 hover:text-blue-600 transition-colors"
                          >
