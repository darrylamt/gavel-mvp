...
export default function SellerOrdersPage() {
  ...
  const confirmDispatch = async () => {
    ...
    try {
      ...
      if (res.ok) {
        notify({ title: 'Dispatched', description: 'Delivery order created with Dawurobo.', variant: 'success' })
        closeModal()
        await loadOrders()
      } else {
        setDispatchError(data.error || 'Failed to create delivery. Please try again.')
      }
    } catch {
      setDispatchError('Failed to create delivery. Please try again.')
    } finally {
            ...
            <div className="divide-y divide-gray-100">
              {orders.map((order) => {
                const statusInfo = order.dawurobo_status
                  ...
                  : null
                const needsDispatch = !order.dawurobo_order_id
                  ...
                  <div key={order.id}>
                    <div className="flex items-center gap-3 px-5 py-4">
                      ...
                      <div className="flex flex-shrink-0 items-center gap-2">
                        ...
                        {needsDispatch && (
                          <button
                            onClick={(e) => { e.stopPropagation(); openDispatchModal(order.id) }}
                        ...
                        <div className="mb-4 grid gap-3 md:grid-cols-2">
                          ...
                          <div className="rounded-2xl border border-gray-100 bg-white p-4">
                            ...
                            {order.dawurobo_order_id ? (
                              ...
                              <p className="text-xs text-gray-500">
                                Not dispatched yet. Use the <span className="font-semibold text-orange-600">Confirm Dispatch</span> button to send this order with Dawurobo.
                              </p>
                            )}
                          </div>
                        </div>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  Order #{modalOrderId.slice(0, 8).toUpperCase()}
                </p>
              </div>
              <button
                onClick={closeModal}
            ...
            <div className="px-5 py-4 space-y-4">
              <p className="text-sm text-gray-600">
                A Dawurobo delivery order will be created for this order. The rider will be dispatched to your pickup address.
              </p>
              <div>
              ...
              {dispatchError && (
                <div className="rounded-xl bg-red-50 border border-red-100 px-3.5 py-2.5 text-sm text-red-700">
                  {dispatchError}
                </div>
              )}
            </div>
            ...
            <div className="flex gap-2 border-t border-gray-100 px-5 py-4">
              ...
              <button
                ...
                disabled={dispatching}
                className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-orange-500 py-2.5 text-sm font-bold text-white hover:bg-orange-600 disabled:opacity-50 transition-colors"
              >
                {dispatching ? (
                  <>
