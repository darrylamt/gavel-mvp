      ...
      {spotlight.map((item) => {
        ...
        return (
          <Link
            key={item.name}
            href={item.href}
            className="group relative overflow-hidden rounded-2xl aspect-[3/4] sm:aspect-[4/5] block"
          ...
          >
            ...
            <span
              className="absolute top-2.5 left-2.5 sm:top-3 sm:left-3 rounded-full px-2 py-0.5 text-xs font-semibold text-white backdrop-blur-sm border"
              style={{ backgroundColor: `${theme.accentHex}30`, borderColor: `${theme.accentHex}60` }}
            >
              {item.badge}
            </span>
