...
import { Calendar, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, LayoutDashboard, Package, Sliders, Smartphone } from 'lucide-react'
...
const tabs = [
  ...
  { href: '/partner/models', label: 'Phone Models', icon: Smartphone },
  { href: '/partner/deductions', label: 'Deduction Rates', icon: Sliders },
]

export default function PartnerShell({ children }: Props) {
      ...
      <div className={`mx-auto grid w-full max-w-7xl gap-4 ${collapsed ? 'lg:grid-cols-[84px_1fr]' : 'lg:grid-cols-[240px_1fr]'}`}>
        <aside className="rounded-2xl bg-white p-4 shadow-sm">
          <div className={`${collapsed ? 'mb-2' : 'mb-4'} flex items-center justify-between`}>
            {!collapsed && (
              <div>
                <h1 className="text-2xl font-bold text-blue-600">Partner</h1>
