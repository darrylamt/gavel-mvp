-- Reconcile older shop checkout schemas with the current payment verification flow.
...
create unique index if not exists idx_shop_orders_paystack_reference
...
alter table public.shop_order_items
  ...
  add column if not exists seller_payout_account_number text,
  add column if not exists seller_payout_provider text,
  add column if not exists variant_id uuid references public.shop_product_variants(id) on delete set null,
  add column if not exists variant_label text;

update public.shop_order_items
set title_snapshot = coalesce(nullif(title_snapshot, ''), 'Product')
where title_snapshot is null or title_snapshot = '';
...
begin
  ...
  if v_has_email_column and v_has_buyer_email_column then
    execute '
      ...
      values ($1, $2, $3, ''paid'', $4, $4, $5, $6, $7, $8, $9)
      returning id
    '
    using p_user_id,
      p_reference,
